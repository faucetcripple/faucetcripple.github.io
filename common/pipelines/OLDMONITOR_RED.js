...
*/
const fragShader = `
#define SHADER_NAME OLDMONITOR_FIXED_FS
...
void main( void ) {
    ...
    vec4 blendA = pixel - vec4( c, 0.0, 0.0, 1.0 );
    vec4 blendB = pixel + vec4( 0.0, c, 0.0, 1.0 );
    vec4 blendC = pixel / vec4( c, c, c, c );
...
export default class OLDMONITOR_FIXED extends Phaser.Renderer.WebGL.Pipelines.PostFXPipeline {
    constructor(game) {
        super({
            game,
            renderTarget: true,
    ...
    onPreRender() {
        ...
        this.increment();
        this.set1f('uSpeed', this._speed / 2);
