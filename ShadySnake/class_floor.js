class Floor {
    ...
    writeInfo(x, y, isTail = true) {
        // this is snake tile-info placement.
...
/*        tmp.setPostPipeline(RBMAT);
    ...
    createFloor() {
        ...
        //        this.infoGrid = mt_2DArray.slice();
        ...
        for (let y = -ny; y <= this.dim.h / 2; y += 1) {
            ...
            for (let x = -nx; x <= this.dim.w / 2; x += 1) {
                ...
                // WIP alpha 
                if (SS.noiseAlphaGrid) {
                    this.alphaTile(tmp, tx, ty);
                }

                // setObjects Properities Accordingly::
                // TINT::
    ...
    updateFloor() { // Working in here::
        ...
        if (SS.noiseAlphaTick) {
            this.activeNoise();
        }

        if (debug.showInfoGrid) {
